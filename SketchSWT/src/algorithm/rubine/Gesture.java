...
public class Gesture
{
    ...
    public void addGesture(gesture.Gesture gesture, boolean resample, boolean rotate, boolean scale, boolean translate) {
    	addGesture(gesture.getDotRepresentation(true), resample, rotate, scale, translate);
    }
    ...
    public void addGesture(List<Dot> gesture, boolean resample, boolean rotate, boolean scale, boolean translate){
		...
		if(it.hasNext()){
			...
			if(!current.hole){
				alp.add(new Point(current.x,current.y));
			}
			while(it.hasNext()){
				Dot last = current;				
				current = it.next();	
				if(!current.hole){
					...
					double v = Math.sqrt((current.x-last.x)*(current.x-last.x) + (current.y-last.y)*(current.y-last.y))/(double)(current.time_millis - last.time_millis);
